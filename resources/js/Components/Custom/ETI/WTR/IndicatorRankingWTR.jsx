...
export default function IndicatorRankingWTR() {
    ...
    const fetchChartData = () => {
        axios.get('/get-eti-wtr-indicator-ranking')
            .then(response => {
                ...
                <table className='min-w-full bg-white border text-sm sm:table hidden'>
                    ...
                    <tbody>
                        {chartData.series.map((seriesItem, seriesIndex) => {
                            const isMainCategory = ['Investment & Development', 'Appeal', 'Readiness'].includes(seriesItem.name);
