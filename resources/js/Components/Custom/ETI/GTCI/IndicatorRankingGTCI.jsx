...
export default function IndicatorRankingGTCI() {
    ...
    const fetchChartData = () => {
        axios.get('/get-eti-gtci-indicator-ranking')
            .then(response => {
                ...
                <table className='min-w-full bg-white border text-sm sm:table hidden'>
                    ...
                    <tbody>
                        {chartData.series.map((seriesItem, seriesIndex) => {
                            const isMainCategory = ['ENABLE', 'ATTRACT', 'GROW', 'RETAIN', 'VOCATIONAL AND TECHNICAL SKILLS', 'GLOBAL KNOWLEDGE SKILLS'].includes(seriesItem.name);
