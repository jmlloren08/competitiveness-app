...
export default function IndicatorRankingPISA() {
    ...
    const availableYears = ['2022', '2018'];
    ...
    const fetchChartData = () => {
        axios.get('/get-eti-pisa-indicator-ranking')
            .then(response => {
                ...
                <table className='min-w-full bg-white border text-sm sm:table hidden'>
                    ...
                    <tbody>
                        {chartData.series.map((seriesItem, seriesIndex) => {
                            const isMainCategory = ['Mathematics literacy', 'Reading literacy', 'Science literacy'].includes(seriesItem.name);
