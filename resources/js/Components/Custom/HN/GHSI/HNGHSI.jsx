...
export default function HNGHSI() {
    const [chartDataHNGHSI, setChartDataHNGHSI] = useState([]);
    const [years, setYears] = useState([]);
    const [countries, setCountries] = useState([]);
    const [loadingHNGHSI, setLoadingHNGHSI] = useState(true);

    useEffect(() => {
        axios.get('/get-hn-ghsi-view-the-ranking')
            .then(response => {
                const { years, countries, data } = response.data;
                setYears(years.map(item => item.ghsi_year));
                setCountries(countries.map(item => item.ghsi_country));
                setChartDataHNGHSI(data);
                setLoadingHNGHSI(false);
            })
            .catch(error => {
    ...
    const series = countries.map(country => ({
        name: country,
        data: chartDataHNGHSI.filter(item => item.ghsi_country === country).map(item => ({
            x: item.ghsi_year,
            y: item.ghsi_count
        }))
    }));
    ...
    const ghsi_economies = ['195 economies', '195 economies'];
                        ...
                        <div key={country} className={`border border-gray-100 rounded-lg mb-4 p-4 ${country === 'Philippines' ? 'bg-blue-300' : 'bg-white'}`}>
                            ...
                            {years.map((year, index) => (
                                <div key={index} className='mb-2'>
                                    ...
                                    <span>
                                        {chartDataHNGHSI.find(item => item.ghsi_country === country && item.ghsi_year === year)?.ghsi_count || 'NDA'}
                                    </span>
                                </div>
                            ))}
                        </div>
                    ...
                    <table className='min-w-full border-collapse'>
                        <thead className='bg-blue-900 text-white'>
                            ...
                            <tr>
                                ...
                                {ghsi_economies.map((economy, index) => (
