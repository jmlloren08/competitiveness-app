...
export default function IndicatorRankingWDCR() {
    ...
    const fetchChartData = () => {
        axios.get('/get-digital-wdcr-indicator-ranking')
            .then(response => {
                ...
                <table className='min-w-full bg-white border text-sm sm:table hidden'>
                    ...
                    <tbody>
                        {chartData.series.map((seriesItem, seriesIndex) => {
                            const isMainCategory = ['Knowledge', 'Technology', 'Future Readiness'].includes(seriesItem.name);
