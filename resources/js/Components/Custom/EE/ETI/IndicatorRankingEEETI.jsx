...
export default function IndicatorRankingEEETI() {
    ...
    const fetchChartData = () => {
        axios.get('/get-ee-eti-indicator-ranking')
            .then(response => {
                ...
                <table className='min-w-full bg-white border text-sm sm:table hidden'>
                    ...
                    <tbody>
                        {chartData.series.map((seriesItem, seriesIndex) => {
                            const isMainCategory = ['System Performance', 'Transition Readiness'].includes(seriesItem.name);
